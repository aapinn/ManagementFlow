...
export default function RecurringIncomeForm() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const hari = Number(tanggalHari)
    if (hari < 1 || hari > 31) return
    ...
    showToast('Pemasukan rutin berhasil ditambahkan')
    resetForm()
    setShowForm(false)
  }

  const active = recurringIncomes.filter((r) => r.aktif)
  const inactive = recurringIncomes.filter((r) => !r.aktif)
    ...
    <div className="card">
      ...
      {showForm && (
        <form onSubmit={handleSubmit} style={{ marginTop: 12, paddingTop: 16, borderTop: '1px solid var(--border-light)' }}>
          ...
          <div className="form-group">
            ...
            <input id="rik" type="text" value={keterangan} onChange={(e) => setKeterangan(e.target.value)} placeholder="Mis: Gaji Bulanan" required />
          </div>
          <div className="form-group">
            <label htmlFor="rikat">Kategori</label>
            <select id="rikat" value={kategori} onChange={(e) => setKategori(e.target.value)} required>
              {INCOME_CATEGORIES.map((c) => <option key={c} value={c}>{c}</option>)}
            </select>
          </div>
          <div className="form-group">
            <label htmlFor="riid">ID Transaksi</label>
            <input id="riid" type="text" value={idTransaksi} onChange={(e) => setIdTransaksi(e.target.value)} placeholder="Masukkan ID transaksi" required />
          </div>
          <div className="form-group">
            <label htmlFor="rihari">Tanggal setiap bulan</label>
            <select id="rihari" value={tanggalHari} onChange={(e) => setTanggalHari(e.target.value)} required>
          ...
          <div className="form-group">
            <label htmlFor="ricat">Catatan <span className="text-muted">(opsional)</span></label>
            <input id="ricat" type="text" value={catatan} onChange={(e) => setCatatan(e.target.value)} placeholder="Catatan tambahan..." />
          </div>
            ...
            <div key={r.id} className="budget-row" style={{ padding: '10px 0' }}>
              ...
              <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginTop: 4 }}>
                ...
                <div style={{ marginLeft: 'auto', display: 'flex', gap: 4 }}>
                  <button
                    className="btn-icon"
                    onClick={() => { updateRecurringIncome(r.id, { aktif: false }); showToast('Pemasukan rutin dinonaktifkan', 'info') }}
                    title="Nonaktifkan"
                  >
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"/><line x1="4.93" y1="4.93" x2="19.07" y2="19.07"/></svg>
                  </button>
                  <button className="btn-icon btn-icon--danger" onClick={() => { deleteRecurringIncome(r.id); showToast('Pemasukan rutin dihapus', 'info') }} title="Hapus">
              ...
              {inactive.map((r) => (
                <div key={r.id} className="budget-row" style={{ padding: '8px 0', opacity: 0.5 }}>
                  ...
                  <div style={{ display: 'flex', gap: 8, marginTop: 4 }}>
                    <span className="cat-tag">{r.kategori}</span>
                    <button className="btn-icon" onClick={() => { updateRecurringIncome(r.id, { aktif: true }); showToast('Pemasukan rutin diaktifkan') }} title="Aktifkan">
      ...
      {recurringIncomes.length === 0 && !showForm && (
        <p className="empty-state" style={{ margin: '16px 0 0' }}>
          Belum ada pemasukan rutin. Tambah untuk otomatis dicatat setiap bulan.
        </p>
      )}
    </div>
  )
